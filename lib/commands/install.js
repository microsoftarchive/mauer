...
function installPackage (package) {
  ...
  if (version) {
    ...
    getPackage(name, version, function (err, cache) {
      ...
      setupPackage(name, version, cache, function () {
...
function getPackage (package, version, callback) {
  ...
  if (fs.existsSync(cachedFile) && md5sum(cachedFile, expectedSum)) {
...
function downloadPackage (url, cachedFile, expectedSum, callback) {
  var stream = fs.createWriteStream(cachedFile);
  request(url, stream, function (err, md5) {
    // var size = fs.statSync(cachedFile).size;
    // size = filesize(size, 2, false);
...
function setupPackage (name, version, cache, callback) {
...
function patchPackage (name, version) {
  ...
  var patchFile = path.resolve(cwd, 'patches', patchName);
...
function deployPackage (name, version) {
  ...
  var lib = path.resolve(cwd, 'public/vendor', name + '.js');
  file.delete(lib);
  file.copy(patched, lib);
}
